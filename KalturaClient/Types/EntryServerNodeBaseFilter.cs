			...
			foreach (XmlElement propertyNode in node.ChildNodes)
			{
				...
				switch (propertyNode.Name)
				{
					...
					case "updatedAtLessThanOrEqual":
						...
						continue;
					case "statusEqual":
						this._StatusEqual = (EntryServerNodeStatus)ParseEnum(typeof(EntryServerNodeStatus), txt);
						continue;
					case "statusIn":
						this._StatusIn = txt;
						continue;
					case "serverTypeEqual":
						this._ServerTypeEqual = (EntryServerNodeType)StringEnum.Parse(typeof(EntryServerNodeType), txt);
						continue;
				}
			}
		}
